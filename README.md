...
All 10 identified issues from the specification have been fixed, verified, and recorded in [`BUGS.md`](./BUGS.md):

1. **Expense List Sorting**: Fixed date parsing in `dateValue` and sorted list to display newest expenses first.
2. **Balances Panel Inversion**: Corrected logic and CSS styling so creditors (`bal > 0`) show as "is owed" (green) and debtors (`bal < 0`) show as "owes" (red).
3. **Non-Participating Payer Reimbursement**: Removed artificial share deductions for payers not included in the expense split.
4. **Equal Debt/Credit Settlements**: Fixed greedy settlement algorithm to record transfers when debt equals credit (`d.amount === c.amount`).
5. **"Paid By" Filter Comparison**: Converted IDs to strings in filter comparison to prevent missing expenses when filtering by payer.
6. **Cent Rounding & Percent Validation**: Allocated exact remainder cents for equal/percentage splits and allowed floating-point tolerance for percentage validation.
7. **LocalStorage Date Hydration**: Ensured `loadState` hydrates date strings into `Date` objects upon reloading from browser local storage.
